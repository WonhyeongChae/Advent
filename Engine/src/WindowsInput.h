...
struct WindowsInputStatus
{
	...
	void Init()
	{
		memset(this, 0, sizeof(WindowsInputStatus));
	}

};

typedef std::vector<InputEvent> WindowsInputEventList;

struct WindowsInputData
{
	WindowsInputStatus mInputStatus;		/// status after all events in mInputEvents
	WindowsInputEventList mInputEvents;	/// every events that occured ince last update
