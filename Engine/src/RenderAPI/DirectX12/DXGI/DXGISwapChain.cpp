...
namespace Engine
{
	...
	void DXGISwapChain::Initialize(ID3D12Device* pDevice, IDXGIFactory2* pFactory, ID3D12CommandQueue* pCommandQueue, const HWND hwnd, const UINT width, const UINT height)
	{
		...
		MY_EVAL_HR(pDevice->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(mRTVHeap.GetAddressOf())), "Error creating the RTV heap");
		...
		MY_EVAL_HR(pFactory->CreateSwapChainForHwnd(pCommandQueue, hwnd, &description, nullptr, nullptr, GetAddressOf()), "Error creating swapchain");
